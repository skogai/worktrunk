...
///
/// `completed` is the only terminal status, so it alone consults `result`; the
/// rest of the documented [build status
/// enum](https://learn.microsoft.com/en-us/rest/api/azure/devops/build/builds/get?view=azure-devops-rest-7.1)
/// (`inProgress`, `notStarted`, `cancelling`, `postponed`) is a run still on
/// its way there and groups as `Running` — matching how
...
mod tests {
    ...
    fn test_parse_azure_pipeline_status() {
        // Non-terminal statuses. "cancelling" is cancellation in flight — the
        // terminal form is "completed" with result "canceled"; "postponed" is a
        // run sitting inactive in the queue.
        for status in ["inProgress", "notStarted", "cancelling", "postponed"] {
            assert_eq!(
                parse_azure_pipeline_status(Some(status), None),
                CiStatus::Running,
                "status={status}"
            );
        }
