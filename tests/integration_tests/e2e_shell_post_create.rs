...
fn test_shell_integration_post_start_background(#[case] shell: &str, repo: TestRepo) {
    ...
    fs::write(
        ...
        r#"post-create = "sleep 0.05 && echo 'Background task done' > bg_marker.txt""#,
    )
    .unwrap();
...
"#,
    ...
    repo.commit("Add multiple post-create commands");
...
#[rstest]
fn test_bash_shell_integration_pre_create_blocks(repo: TestRepo) {
    ...
    repo.commit("Add pre-create command");
...
"#,
    ...
    // Verify that pre-create command completed before wt returned (blocking behavior)
    // The file should exist immediately after wt exits
    let setup_file = worktree_path.join("setup.txt");
    assert!(
        setup_file.exists(),
        "Setup file should exist immediately after wt returns (pre-create is blocking)"
    );
