...
- [Dev server per worktree](https://worktrunk.dev/tips-patterns/#dev-server-per-worktree): `hash_port` in `post-create` for launch and `post-remove` for cleanup, with optional subdomain routing
